...
default_cfgs = {
    ...
    'vit_small_patch16_224': _cfg(
        url='./weights/vit_small_p16_224-15ec54c9.pth',
    ),
...
class VisionTransformer(nn.Module):
    ...
    def relprop(self, cam=None, method="transformer_attribution", is_ablation=False, start_layer=0, **kwargs):
        ...
        if method == "full":
            ...
            # print("This is what you want:",cam.shape)
            # cam = 1,197,768[:,1:] -> 1,196,768
            cam = cam[:, 1:]
            # print(cam.shape)
            #cam = 1,3,224,224
            cam = self.patch_embed.relprop(cam, **kwargs)
            # print("Patch Embed:",cam.shape)
            # sum on channels
            #cam = 1,224,224
            cam = cam.sum(dim=1)
            return cam
            ...
            attn_cams = []
            for blk in self.blocks:
                # attn_heads = relu(1,12,197,197)
                attn_heads = blk.attn.get_attn_cam().clamp(min=0)
                # print("attn_heads", attn_heads.shape)
                # avg_heads = mean(1,12,197,197) -> 1,197,197
                avg_heads = (attn_heads.sum(dim=1) / attn_heads.shape[1]).detach()
                # print("avg_heads", avg_heads.shape)
                attn_cams.append(avg_heads)
            # attn_cams = #layers,197,197
            # cam = 1,197,197 
        ...
        # beyond attention
            ...
            cams = []
            for blk in self.blocks:
                # grad = cam = 1,12,197,197
                grad = blk.attn.get_attn_gradients()
                cam = blk.attn.get_attn_cam()
                # grad = cam = 12,197,197
                cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
                grad = grad[0].reshape(-1, grad.shape[-1], grad.shape[-1])
                # print("this is ",cam.shape, grad.shape)
                # cam = 12,197,197
                cam = grad * cam
                # cam = relu(12,197,197)
                # cam = mean(12,197,197)-> 197,197
                # Mean over heads
            ...
            if self.mae:
                # cam = mean(1,196,196) -> 1,196
                # Mean over vectors
                cam = rollout[:, 1:, 1:].mean(1)
            elif self.dino:
                # cam = mean(1,196,196,dim=1) + 1,1,196
                # Mean over vectors + CLS token's vector
        ...
        # Last layer's grad*relevance
            ...
            cam = cam.clamp(min=0).mean(dim=0)
            if self.mae:
                # cam  = mean([196,196],dim=0) -> 1,196
                #mean of patches removing CLS token
                cam = cam[1:, 1:].mean(0)
            elif self.dino:
                # cam  = mean([196,196],dim=0) + 1,196
                #mean of patches removing CLS token + CLS token
                cam = cam[1:, 1:].mean(0) + cam[0, 1:]
            else:
                # cam = 1,196
                # CLS token's vector as cam
                cam = cam[0, 1:]
            return cam

        # Last layer's attention only
            ...
            #cam = 12,197,197
            cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
            # cam = relu(12,197,197).mean(0) -> 197,197
            # Mean over heads
            cam = cam.clamp(min=0).mean(dim=0)
            # cam = 1,196
            # CLS token's vector as cam
        ...
        # Second layer's grad*relevance
        elif method == "second_layer":
            #cam = 1,12,197,197 -> 12,197,197
            cam = self.blocks[1].attn.get_attn_cam()
            cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
            if is_ablation:
                grad = self.blocks[1].attn.get_attn_gradients()
...
def load_weights(path: str, patch_size: int = 16) -> OrderedDict:
    out_dict = {}
...
if __name__ == "__main__":
    ...
    image = read_image("sample.jpeg")
    output = model(image)
    # accum_grad(output,1,model)
    # print(model.blocks[0].attn.get_attgradients())
